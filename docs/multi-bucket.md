...
5. 点击“添加并切换”，连接验证通过后会在当前页面切换到新 Bucket。
...
切换 Bucket 不再刷新整个网页，而是执行以下流程：

1. 暂时锁定 Bucket 选择器和文件操作区域；
2. 使用目标 Bucket 配置执行一次最小连接验证；
3. 验证成功后关闭旧预览、退出批量选择，并回到根目录；
4. 清理文件列表缓存，只重新加载文件浏览区域；
5. 验证失败时恢复原 Bucket，保留当前文件列表。

为避免操作串到错误的 Bucket，在上传、复制、移动、删除或创建文件夹期间会禁止切换。任务完成后即可切换。
